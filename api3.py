...
def create_measurement():
    ...
    try:
        ...
        for items in data:
            ...
            if missing_fields:
                return jsonify({"error": f"Missing required field: {', '.join(missing_fields)}"}), 400
...
def modify_multiple_measurements():
    ...
    try:
        ...
        if not isinstance(data, list):
            return jsonify({"error": "Request body must be a list of measurement updates"}), 400
        ...
        for item in data:
            ...
            if measurement_id in duplicate_id:
                return jsonify({"error": "Duplicate ID found in request"}), 400
